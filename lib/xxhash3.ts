 ...
 */
export async function xxhash3(data: IDataType, seedLow = 0, seedHigh = 0): Promise<string> {
 ...
 *  initialize the internal state of the algorithm (defaults to 0)
 */
export async function createXXHash3(seedLow = 0, seedHigh = 0): Promise<IHasher> {
  validateSeed(seedLow);
  validateSeed(seedHigh);
