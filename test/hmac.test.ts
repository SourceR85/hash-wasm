...
async function HMACTest(key: IDataType, data: IDataType): Promise<void> {
  ...
  expect(await getHashWasmHMAC(sha256Hasher, key, data)).toBe(getNodeHMAC('sha256', key, data));
  expect(await getHashWasmHMAC(sha384Hasher, key, data)).toBe(getNodeHMAC('sha384', key, data));
  expect(await getHashWasmHMAC(sha512Hasher, key, data)).toBe(getNodeHMAC('sha512', key, data));
  expect(await getHashWasmHMAC(sha3224Hasher, key, data)).toBe(getNodeHMAC('sha3-224', key, data));
  expect(await getHashWasmHMAC(sha3256Hasher, key, data)).toBe(getNodeHMAC('sha3-256', key, data));
  expect(await getHashWasmHMAC(sha3384Hasher, key, data)).toBe(getNodeHMAC('sha3-384', key, data));
...
test('works with different key type', async () => {
  ...
  await HMACTest(new Uint8Array(buf.buffer, buf.byteOffset, buf.length), 'data');
});
