...
const xxhash3 = async (
  data: IDataType,
): Promise<string> => origXXHash3(data, 0x76543210, 0xFEDCBA98);
...
test('different seeds give different response', async () => {
  ...
  expect(await origXXHash3('', 0xAB)).not.toBe(await origXXHash3('', 0xAB, 0x0C));
  expect(await origXXHash3('', 0xAB, 0x0D)).not.toBe(await origXXHash3('', 0xAB, 0x0C));
  expect(await origXXHash3('', 0, 0x0D)).not.toBe(await origXXHash3('', 0, 0x0C));
  expect(await origXXHash3('', 0, 0)).not.toBe(await origXXHash3('', 1, 0));
  expect(await origXXHash3('', 0, 0)).not.toBe(await origXXHash3('', 0, 1));
  expect((await createXXHash3(1, 2)).init().update('').digest()).toBe(await origXXHash3('', 1, 2));
  expect((await createXXHash3(1, 2)).init().update('').digest()).not.toBe(await origXXHash3('', 1, 3));
  expect((await createXXHash3(0, 2)).init().update('').digest()).not.toBe(await origXXHash3('', 0, 1));
  expect((await createXXHash3(1, 0)).init().update('').digest()).not.toBe(await origXXHash3('', 0, 0));
});
...
test('simple strings', async () => {
  ...
  expect(await xxhash3('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789')).toBe('30569891b862289b');
  expect(await xxhash3('12345678901234567890123456789012345678901234567890123456789012345678901234567890')).toBe('6e20719f479d9acb');
});
...
test('Node.js buffers', async () => {
  ...
  expect(await xxhash3(Buffer.from(['a'.charCodeAt(0)]))).toBe('42901760fdc9d406');
  expect(await xxhash3(Buffer.from([0]))).toBe('76761c489ed890a3');
  expect(await xxhash3(Buffer.from([0, 1, 0, 0, 2, 0]))).toBe('7263b8ce0c0a4cd7');
});
...
test('Invalid inputs throw', async () => {
  ...
  invalidInputs.forEach(async (input: any) => {
    await expect(origXXHash3(input)).rejects.toThrow();
