...
test('simple keys', async () => {
  ...
  expect(await blake2b('', 256, 'this is the password')).toBe('386435aaf3ba280ea9b3ebd7b42d9c5bbf63f23dbae9ab91553332a588fe337c');
...
test('unicode keys', async () => {
  expect(await blake2b('a', 256, '😊')).toBe('43a791ea899560fc25e3a18eaa11f85b640a6802e7c94b08ac51f0efe39806c9');
  expect(await blake2b('😊a😊', 256, '😊a😊')).toBe('0dcc9cd4672ad13d447a3f64399d20d5ac8b2c835746b570c1e3ff4d735dfc96');
  expect(await blake2b('a', 512, '😊')).toBe('662f46f559501f4fb780bba0a410c8a5bb58f646a02b1361aeb9f0c9c29dd3faf99db0abc6132d302e5e07cec147cc0359cf0c4568e6bfc6455fe12e3eed6d5b');
  expect(await blake2b('😊a😊', 512, '😊a😊')).toBe('5e644d389195eb2fa99227fbcde278f290d8a02c50202fa59b71a53d5d0810b03cfaff7e609b57daba9fecb8ab9beabf75f42ce263dfaaf0bd6bb84255dec16a');
});
...
test('typed arrays', async () => {
  ...
  expect(await blake2b('a', 256, Buffer.from(key))).toBe('3584ad664f76aaa768424a8a125d3052a9ea5bc90c18c06ffba60e980f2d9f3a');
...
test('Invalid keys throw', async () => {
  await expect(blake2b('a', 256, 0 as any)).rejects.toThrow();
  await expect(blake2b('a', 256, 1 as any)).rejects.toThrow();
  await expect(blake2b('a', 256, Number(1) as any)).rejects.toThrow();
  await expect(blake2b('a', 256, {} as any)).rejects.toThrow();
  await expect(blake2b('a', 256, [] as any)).rejects.toThrow();
  await expect(createBLAKE2b(256, 0 as any)).rejects.toThrow();
  await expect(createBLAKE2b(256, 1 as any)).rejects.toThrow();
  await expect(createBLAKE2b(256, Number(1) as any)).rejects.toThrow();
  await expect(createBLAKE2b(256, {} as any)).rejects.toThrow();
  await expect(createBLAKE2b(256, [] as any)).rejects.toThrow();
});
