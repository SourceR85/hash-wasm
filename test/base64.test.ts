...
test('encode-decode', () => {
  for (let i = 800; i < 1050; i++) {
    let buf = Buffer.alloc(i, 0xFF);
    expect(toHex(decodeBase64(encodeBase64(buf, false)))).toStrictEqual(buf.toString('hex'));
    expect(toHex(decodeBase64(encodeBase64(buf, true)))).toStrictEqual(buf.toString('hex'));
    buf = Buffer.alloc(i, 0x00);
    expect(toHex(decodeBase64(encodeBase64(buf, false)))).toStrictEqual(buf.toString('hex'));
    expect(toHex(decodeBase64(encodeBase64(buf, true)))).toStrictEqual(buf.toString('hex'));
    buf = Buffer.alloc(i, i % 256);
