...
test('bcrypt self-test', async () => {
  ...
  const salt = Buffer.from([
    113, 215, 159, 130, 24, 163, 146, 89, 167, 162, 154, 171, 178, 219, 175, 195,
  ]);

  expect(
    await hash(pass, salt, 4, 'encoded'),
  ).toBe('$2a$04$abcdefghijklmnopqrstuupykUmEpWFIodLTMbMX8aLgIbIjXE2ye');

  expect(
    await hash(pass, salt, 5, 'encoded'),
  ).toBe('$2a$05$abcdefghijklmnopqrstuuBBVy3wTnmAwUGHhsQHECk9dvWK.8Kaa');

  expect(
    await hash(pass, salt, 6, 'encoded'),
  ).toBe('$2a$06$abcdefghijklmnopqrstuuzwLuAZdPqTLZf34qmCGDt1OORDMmDcC');

  expect(
    await hash(pass, salt, 14, 'encoded'),
  ).toBe('$2a$14$abcdefghijklmnopqrstuuEKD8s8kkQZnOwmJAaMXPDP.L249l7n6');
});

test('bcrypt bundled tests', async () => {
  expect(
    await hash('U*U', Buffer.from([16, 65, 4, 16, 65, 4, 16, 65, 4, 16, 65, 4, 16, 65, 4, 16]), 5, 'encoded'),
  ...
  expect(
    await hash('U*U*U', Buffer.from([101, 150, 89, 101, 150, 89, 101, 150, 89, 101, 150, 89, 101, 150, 89, 101]), 5, 'encoded'),
  ).toBe('$2a$05$XXXXXXXXXXXXXXXXXXXXXOAcXxm9kjPGEMsLznoKqmqw7tc8WCx4a');

  expect(
    await hash(
      '0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789',
      Buffer.from([113, 215, 159, 130, 24, 163, 146, 89, 167, 162, 154, 171, 178, 219, 175, 195]),
      5,
      'encoded',
    ),
  ).toBe('$2a$05$abcdefghijklmnopqrstuu5s2v8.iXieOjg/.AySBTTZIIVFJeBui');

  const salt = Buffer.from([5, 3, 0, 133, 213, 237, 76, 23, 107, 42, 195, 203, 238, 71, 41, 28]);
  expect(
    await hash(Buffer.from([0xff, 0xff, 0xa3]), salt, 5, 'encoded'),
  ).toBe('$2a$05$/OK.fbVrR/bpIqNJ5ianF.nqd1wy.pTMdcvrRWxyiGL2eMz.2a85.');

  expect(
    await hash(Buffer.from([0xa3]), salt, 5, 'encoded'),
  ).toBe('$2a$05$/OK.fbVrR/bpIqNJ5ianF.Sa7shbm4.OzKpvFnX1pQLmQW96oUlCq');
  ...
  expect(
    await hash(Buffer.from([0xff, 0xa3, 0x33, 0x34, 0xff, 0xff, 0xff, 0xa3, 0x33, 0x34, 0x35]), salt, 5, 'encoded'),
  ...
  expect(
    await hash(Buffer.from([0xa3, 0x61, 0x62]), salt, 5, 'encoded'),
  ).toBe('$2a$05$/OK.fbVrR/bpIqNJ5ianF.6IflQkJytoRVc1yuaNtHfiuq.FRlSIS');

  expect(
    await hash(Buffer.alloc(72, 0xaa), salt, 5, 'encoded'),
  ).toBe('$2a$05$/OK.fbVrR/bpIqNJ5ianF.swQOIzjOiJ9GHEPuhEkvqrUyvWhEMx6');
  ...
  expect(
    await hash(Buffer.alloc(72, Buffer.from([0x55, 0xaa, 0xff])), salt, 5, 'encoded'),
...
test('Invalid bcrypt verify parameters', async () => {
  ...
  await testHash('@2a$06$KRGxLBS0Lxe3KBCwKxOzLeUQ0eaAQoaT9eYD/M6ixOkZwzuuCPPwO');
  await testHash('$1a$06$KRGxLBS0Lxe3KBCwKxOzLeUQ0eaAQoaT9eYD/M6ixOkZwzuuCPPwO');
  await testHash('$2c$06$KRGxLBS0Lxe3KBCwKxOzLeUQ0eaAQoaT9eYD/M6ixOkZwzuuCPPwO');
  await testHash('$2z$06$KRGxLBS0Lxe3KBCwKxOzLeUQ0eaAQoaT9eYD/M6ixOkZwzuuCPPwO');
  await testHash('$22$06$KRGxLBS0Lxe3KBCwKxOzLeUQ0eaAQoaT9eYD/M6ixOkZwzuuCPPwO');
  await testHash('$2a@06$KRGxLBS0Lxe3KBCwKxOzLeUQ0eaAQoaT9eYD/M6ixOkZwzuuCPPwO');
  await testHash('$2a$03$KRGxLBS0Lxe3KBCwKxOzLeUQ0eaAQoaT9eYD/M6ixOkZwzuuCPPwO');
  await testHash('$2a$00$KRGxLBS0Lxe3KBCwKxOzLeUQ0eaAQoaT9eYD/M6ixOkZwzuuCPPwO');
  await testHash('$2a$32$KRGxLBS0Lxe3KBCwKxOzLeUQ0eaAQoaT9eYD/M6ixOkZwzuuCPPwO');
  await testHash('$2a$06@KRGxLBS0Lxe3KBCwKxOzLeUQ0eaAQoaT9eYD/M6ixOkZwzuuCPPwO');
  await testHash('$2a$06$@RGxLBS0Lxe3KBCwKxOzLeUQ0eaAQoaT9eYD/M6ixOkZwzuuCPPwO');
  await testHash('$2a$06$KRGxLBS0Lxe3KBCwKxOzLeUQ0eaAQoaT9eYD/M6ixOkZwzuuCPPw@');
  await testHash('$2a$06$KRGxLBS0Lxe3KBCwKxOzLeUQ0eaAQoaT9eYD/M6ixOkZwzuuCPPwO2');
});
