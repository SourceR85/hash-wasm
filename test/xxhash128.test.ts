...
const xxhash128 = async (data: IDataType) => origXXHash128(data, 0x76543210, 0xFEDCBA98);
...
test('simple strings with 0 seed', async () => {
  ...
  expect(await origXXHash128('1234567890')).toBe('82d9f70aeb974c48565e705734e91277');
});

test('simple strings with FF seed', async () => {
  expect(await origXXHash128('', 0xFF)).toBe('cdffe4eccab82d80b8f58ddafc3701e9');
  expect(await origXXHash128('a', 0xFF)).toBe('8d3a71916c1eba63823451005b75c214');
  expect(await origXXHash128('1234567890', 0xFF)).toBe('fdeb6e88caea8b896ef69089eb03093d');
});

test('simple strings with ABCD seed', async () => {
  expect(await origXXHash128('', 0xABCD)).toBe('1bc0c8603f8f37eaef324444feed1c13');
  expect(await origXXHash128('a', 0xABCD)).toBe('4439c290759639040ccd599d595f9d58');
  expect(await origXXHash128('1234567890', 0xABCD)).toBe('8d8644c8806db1f7cd7e5e69c092dbe7');
});

test('different seeds give different response', async () => {
  expect(await origXXHash128('', 0xAB)).not.toBe(await origXXHash128('', 0xAC));
  expect(await origXXHash128('', 0xAB)).not.toBe(await origXXHash128('', 0xAB, 0x0C));
  expect(await origXXHash128('', 0xAB, 0x0D)).not.toBe(await origXXHash128('', 0xAB, 0x0C));
  expect(await origXXHash128('', 0, 0x0D)).not.toBe(await origXXHash128('', 0, 0x0C));
  ...
  expect((await createXXHash128(1, 2)).init().update('').digest()).toBe(await origXXHash128('', 1, 2));
  expect((await createXXHash128(1, 2)).init().update('').digest()).not.toBe(await origXXHash128('', 1, 3));
  expect((await createXXHash128(0, 2)).init().update('').digest()).not.toBe(await origXXHash128('', 0, 1));
  expect((await createXXHash128(1, 0)).init().update('').digest()).not.toBe(await origXXHash128('', 0, 0));
});
...
test('simple strings', async () => {
  ...
  expect(await xxhash128('1234567890')).toBe('50b0fcf3cf8a6a7b86b30e7917971068');
  ...
  expect(await xxhash128('message digest')).toBe('762dad1152fca7505465f1ab077f3ab6');
  expect(await xxhash128('abcdefghijklmnopqrstuvwxyz')).toBe('972cd1cc9b1db75f7a359825199acf86');
  expect(await xxhash128('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789')).toBe('f5c371a7965b7c0173b0ae72b3936b6f');
  expect(await xxhash128('12345678901234567890123456789012345678901234567890123456789012345678901234567890')).toBe('4a852ab07607c14f6cbeb984312460bb');
});
...
test('unicode strings', async () => {
  ...
  expect(await xxhash128(file.toString())).toBe('52d2646c246d67bdaca6bbc548f4aa6b');
});

test('Node.js buffers', async () => {
  expect(await xxhash128(Buffer.from([]))).toBe('258c3140e9acde456f0610acd4631b3d');
  expect(await xxhash128(Buffer.from(['a'.charCodeAt(0)]))).toBe('ec6c741ea572296c42901760fdc9d406');
  expect(await xxhash128(Buffer.from([0]))).toBe('a09afc61a0c767fe76761c489ed890a3');
  expect(await xxhash128(Buffer.from([0, 1, 0, 0, 2, 0]))).toBe('9435bb9864f63027c1ba5284bd6915c3');
});

test('typed arrays', async () => {
  const arr = [0, 1, 2, 3, 4, 5, 255, 254];
  expect(await xxhash128(Buffer.from(arr))).toBe('ef12519ad92140e711a754d7b5a36cbd');
  const uint8 = new Uint8Array(arr);
  expect(await xxhash128(uint8)).toBe('ef12519ad92140e711a754d7b5a36cbd');
  expect(await xxhash128(new Uint16Array(uint8.buffer))).toBe('ef12519ad92140e711a754d7b5a36cbd');
  expect(await xxhash128(new Uint32Array(uint8.buffer))).toBe('ef12519ad92140e711a754d7b5a36cbd');
});
