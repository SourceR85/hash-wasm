...
test('simple strings', async () => {
  ...
  expect(await blake2s('message digest', 128)).toBe('a120dbd782f5e524252ba9e77e69301b');
  expect(await blake2s('abcdefghijklmnopqrstuvwxyz', 128)).toBe('6b5da6a19a600add9fada4c0b95bf6c9');
  expect(await blake2s('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789', 128)).toBe('ae8812ea7e3507014d764e3d1f57387e');
  expect(await blake2s('12345678901234567890123456789012345678901234567890123456789012345678901234567890', 128)).toBe('d0b88b4a58efa805a1f7642865edd050');
});
...
test('unicode strings', async () => {
  ...
  expect(await blake2s(file.toString(), 128)).toBe('2c5f7941cd083e25fa0b6e0eab371f40');
});

test('Node.js buffers', async () => {
  expect(await blake2s(Buffer.from([]), 128)).toBe('64550d6ffe2c0a01a14aba1eade0200c');
  expect(await blake2s(Buffer.from(['a'.charCodeAt(0)]), 128)).toBe('854b9e9ba49bfd9457d4c3bf96e42523');
  expect(await blake2s(Buffer.from([0]), 128)).toBe('9f31f3ec588c6064a8e1f9051aeab90a');
  expect(await blake2s(Buffer.from([0, 1, 0, 0, 2, 0]), 128)).toBe('7c357b1444b9b7f89c7cd4e72ff10c36');
});

test('typed arrays', async () => {
  const arr = [0, 1, 2, 3, 4, 5, 255, 254];
  expect(await blake2s(Buffer.from(arr), 128)).toBe('2c64964b70aad235398456062b3984fb');
  const uint8 = new Uint8Array(arr);
  expect(await blake2s(uint8, 128)).toBe('2c64964b70aad235398456062b3984fb');
  expect(await blake2s(new Uint16Array(uint8.buffer), 128)).toBe('2c64964b70aad235398456062b3984fb');
  expect(await blake2s(new Uint32Array(uint8.buffer), 128)).toBe('2c64964b70aad235398456062b3984fb');
});
...
test('Invalid inputs throw', async () => {
  ...
  invalidInputs.forEach(async (input: any) => {
    await expect(blake2s(input, 128)).rejects.toThrow();
