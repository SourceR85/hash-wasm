...
test('simple strings', async () => {
  ...
  expect(await blake2b('message digest', 128)).toBe('a235c121347fdd24feffe048dbe68ccc');
  expect(await blake2b('abcdefghijklmnopqrstuvwxyz', 128)).toBe('82a82a043c4946fa81b9a598a3e8d35b');
  expect(await blake2b('ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz0123456789', 128)).toBe('38d187cd913684abd1d08f63337bdde1');
  expect(await blake2b('12345678901234567890123456789012345678901234567890123456789012345678901234567890', 128)).toBe('3242cc3901ffad79cb164104a9486881');
});
...
test('unicode strings', async () => {
  ...
  expect(await blake2b(file.toString(), 128)).toBe('bfa7e10f39f0120c640b0c09122a13b2');
});

test('Node.js buffers', async () => {
  expect(await blake2b(Buffer.from([]), 128)).toBe('cae66941d9efbd404e4d88758ea67670');
  expect(await blake2b(Buffer.from(['a'.charCodeAt(0)]), 128)).toBe('27c35e6e9373877f29e562464e46497e');
  expect(await blake2b(Buffer.from([0]), 128)).toBe('7025e075d5e2f6cde3cc051a31f07660');
  expect(await blake2b(Buffer.from([0, 1, 0, 0, 2, 0]), 128)).toBe('05b74966098218dd7a4e538981ac2984');
});

test('typed arrays', async () => {
  const arr = [0, 1, 2, 3, 4, 5, 255, 254];
  expect(await blake2b(Buffer.from(arr), 128)).toBe('9533446a2d7e4ebd36f196ab18ff5040');
  const uint8 = new Uint8Array(arr);
  expect(await blake2b(uint8, 128)).toBe('9533446a2d7e4ebd36f196ab18ff5040');
  expect(await blake2b(new Uint16Array(uint8.buffer), 128)).toBe('9533446a2d7e4ebd36f196ab18ff5040');
  expect(await blake2b(new Uint32Array(uint8.buffer), 128)).toBe('9533446a2d7e4ebd36f196ab18ff5040');
});
